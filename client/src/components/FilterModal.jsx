  ...
  return (
    <div className="fixed h-screen w-screen top-0 left-0 bg-gray-700 flex justify-center items-center bg-opacity-50 backdrop-blur-sm">
      <form className="card min-w-64">
        <input type="text" value={searchParams.get("key")} className="hidden" name="key" />
        <h1 className="mb-4">Filter</h1>
        <section>
          <label htmlFor="category" className="block">
            Category
          </label>
          <select name="category" id="category" className="w-full" defaultValue={searchParams.get("category") || ""}>
        ...
        <section>
          ...
          <select name="status" id="status" className="w-full" defaultValue={searchParams.get("status") || ""}>
